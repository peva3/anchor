...
<!-- HUMAN: These sections flag non-obvious decisions and areas needing scrutiny.
      They are review aids, NOT authorship attribution — all work is authored by
      the human (Sections 2, 36.5 forbid attributing work to an AI). -->
...
| **Issue links** | RECOMMENDED | `Fixes #123` / `Closes #456` so the PR is discoverable and auto-closes on merge |
| **CI run link** | RECOMMENDED | Link the checks run so reviewers see green without hunting |
| **Rollback plan** | REQUIRED for risky changes | How to revert or feature-flag this if it misbehaves |
| **Agent disclosure** | REQUIRED for AI-assisted PRs | Review-assistance notes (decisions, areas to scrutinize) — never authorship attribution, per Sections 2/36.5 |
